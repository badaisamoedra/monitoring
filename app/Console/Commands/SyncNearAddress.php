...
class SyncNearAddress extends Command
{
	/**
	 ...
	 * @var string
	 */
	protected $signature = 'sync_near_address:sync';
	 ...
	 * @var string
	 */
	protected $description = 'sync near address at 02.00';
		...
		try{
			...
			if(!empty($needToGetFromOpenStreet)){
				foreach($needToGetFromOpenStreet as $val){
					...
					$geocodeFromLatLong = file_get_contents('https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat='.trim($val['latitude']).'&lon='.trim($val['longitude']).'&limit=1&email=fejaena.3@gmail.com'); 
			...
			$saveLogs = [
				'status' 		 => 'SUCCESS',
				'file_function'  => 'New Sync Address (Start = '. $start_time.' End = '.date('Y-m-d H:i:s').')',
